...
def get_BioBert(location):
    ...
    if location == 'dropbox':
        if not os.path.isdir(model_path):
            ...
            if not os.path.exists(model_path / "pytorch_model.bin"):
                print("Extracting biobert_embedding model tar.gz")
                tar = tarfile.open(model_path / "biobert_v1.1_pubmed_pytorch_model.tar.gz")
    ...
    if location == 'google drive':
        if not os.path.isdir(model_path):
            os.makedirs(model_path)
            print("Downloading the biobert_embedding model, will take a minute...")
...
if __name__ == "__main__":
    print("package from downloading biobert_embedding model")
